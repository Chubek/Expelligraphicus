pub mod context {
    ...
    use crate::bitmap::bitmap::bitmap::Canvas;
            ...
            for (i, v) in map.clone().into_iter().enumerate() {
                for (j, c) in v.into_iter().enumerate() {
                    let color = match c.color_type {
                        crate::bitmap::color::color::ColorType::RGB => {
                            let (r, g, b) = c.unravel_rgb();

                            pixels::Color::RGB(r, g, b)
                        },
                        crate::bitmap::color::color::ColorType::RGBA => {
                    ...
                    canvas.draw_point(rect::Point::new(
                        w  as i32 - i as i32, h  as i32 - j as i32)).unwrap();
                }
            }
